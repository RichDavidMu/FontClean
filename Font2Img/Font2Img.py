...
def draw_txt(n, charset,  size):
    ...
    for i in range(n):
        ...
        font = ImageFont.truetype(font_dir, int(img_h * factor), 0)
        # 获取当前字体下的文本区域大小
        ...
        while (text_size[0] > margin[0]) or (text_size[1] > margin[1]):
            factor -= 0.01  # 控制字体大小
            font = ImageFont.truetype(font_dir, int(img_h * factor), 0)  # 加载字体
            text_size = font.getsize(char)
